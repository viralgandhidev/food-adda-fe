...
export default function SubscribePage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-white flex flex-col">
      ...
      />
      <Header />
      <main className="flex-1 px-6 md:px-[135px] pt-12 pb-20">
        {subLoading ? (
            ...
            {subscription.status === "PENDING_PAYMENT" ? (
              <>
                <p className="mt-4 text-sm text-gray-600">
                  Your subscription payment is pending. Please complete the
                  payment to activate your subscription.
                </p>
                <button
                  onClick={() =>
                    subscription.plan_code &&
                    startCheckout(subscription.plan_code)
                  }
                  disabled={loadingPlan === subscription.plan_code}
                  className="mt-6 inline-flex rounded-full bg-[#F4D300] px-6 py-2 font-semibold text-[#181818] hover:bg-[#F6DD3D] disabled:opacity-60"
                >
                  {loadingPlan === subscription.plan_code
                    ? "Starting..."
                    : "Complete Payment"}
                </button>
              </>
          ...
          <div className="grid md:grid-cols-[minmax(280px,1fr)_360px] gap-10 items-start">
