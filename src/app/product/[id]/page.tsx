...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/products/${id}`).then(async (res) => {
      ...
      setProduct(prod);
      // Only attempt saved checks if user is authenticated
      try {
        const token =
            ...
            <div className="mt-4">
              <div className="flex items-center gap-3">
                ...
                {!isLoggedIn && (
                  <Link
                    href="/login"
                    className="text-xs font-semibold text-[#181818] bg-[#F4D300] px-3 py-1 rounded-full shadow hover:bg-yellow-400 transition"
                  >
                    Log in to view
                  </Link>
                )}
